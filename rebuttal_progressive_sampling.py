...
def split_data(filename):
    ...
    for c in xrange(len(sortpdcontent)):
        content.append(solution_holder(
                                       ...
                                       [1/(float(i) + 0.000001) for i in sortpdcontent.iloc[c][depcolumns].tolist()],
                                       c
                                       )
                       )
...
if __name__ == "__main__":
    ...
    rank_diff_collector = {}
    for file in files:
        rank_diff_collector[file] = {}
        rank_diff_collector[file]['rank_rd'] = []
        rank_diff_collector[file]['mmre_rd'] = []
        rank_diff_collector[file]['rank_eval'] = []
        rank_diff_collector[file]['mmre_eval'] = []
        print file
        mres = []
        sizes = []
        wins = 0
        ...
        for _ in xrange(20):
            ...
            rank_diff_collector[file]['rank_rd'].append(min_rank_prog)
            rank_diff_collector[file]['rank_eval'].append(len_rank_train_set)
            ...
            rank_diff_collector[file]['mmre_rd'].append(min_mre_prog)
            rank_diff_collector[file]['mmre_eval'].append(len_mre_train_set)
        ...
        import pickle
        pickle.dump(rank_diff_collector, open('rebuttal_rank_diff'+ file.split('/')[-1][:-4] + '.p', 'w'))
