...
print sorted([[i, (d/proj)*100] for i, (d, proj) in enumerate(zip(dumb_evals, atri_evals))], key=lambda x:x[-1])
...
fig, ax = plt.subplots()
ax.plot([i for i in xrange(3, 110)], [100 for _ in xrange(3, 110)], linestyle='--', color='black', label='Projective Sampling')
