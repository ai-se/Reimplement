...
N = len(data)
dumb_mres = [d[1] for d in data]
dumb_stds = [d[2] for d in data]
...
rects2 = ax.bar(ind + 1 * width, random_mres, width, color='#bdbdbd', log=True, label='Progessive Sampling')

atri_mres = [d[7] for d in data]
atri_stds = [d[8] for d in data]
rects3 = ax.bar(ind + 2 * width, atri_mres, width, color='#636363', log=True, label='Projective Sampling')
...
ax.set_ylabel('MMRE', fontsize=16)
# ax.set_title('Scores by group and gender')

ax.set_xticks(ind + 3*width / 2)
ax.set_xticklabels(['SS'+str(x+1) for x in xrange(len(data))], rotation='vertical', fontsize=16)
